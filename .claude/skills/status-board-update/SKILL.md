...
description: Maintain your status board at status-board.md as the single source of truth for active work. Read it on session start. Update it the same turn you act on a tracked entity. Move resolved rows to archived. Use whenever a tracked client, project, task, or thread changes state.
---

# status-board-update

The status board at `status-board.md` is the single canonical view of what is currently happening in your work. Treat it as zero-class infrastructure. Every other knowledge file in the corpus is reference material; this one is live state.
...
2. Whenever you references "what is going on with X" or asks for an overview of their work.
...
- Do not let status drift more than a session without an update. That is the failure shape that produces the stale memorythey pays you to prevent.
